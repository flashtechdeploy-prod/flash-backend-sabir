...
async def _ensure_client_name_is_text() -> None:
    if engine.dialect.name != "postgresql":
        return

    async with engine.begin() as conn:
        try:
            await conn.execute(text("ALTER TABLE clients ALTER COLUMN client_name TYPE TEXT"))
        except Exception:
            pass
...
async def _seed_rbac() -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            async def _next_id(model_cls) -> int:
                try:
                    result = await db.execute(select(func.max(model_cls.id)))
                    cur = result.scalar()
            ...
            for key, desc in permissions.items():
                ...
                if not exists:
                    ...
                    if getattr(row, "id", None) is None:
                        row.id = await _next_id(Permission)
                    db.add(row)
            await db.commit()
            print("[SEED] Permissions seeded successfully")

            result = await db.execute(select(Permission))
            perm_by_key = {p.key: p for p in result.scalars().all()}
            print(f"[SEED] Found {len(perm_by_key)} permissions")
            ...
            for role_name, spec in roles.items():
                ...
                role_perms = [perm_by_key[k] for k in spec.get("permissions", []) if k in perm_by_key]
                r.permissions = role_perms
            await db.commit()
            print("[SEED] Roles seeded successfully")

            result = await db.execute(select(Role))
            role_by_name = {r.name: r for r in result.scalars().all()}
            print(f"[SEED] Found {len(role_by_name)} roles")
            ...
            for username, spec in users.items():
                ...
                if not u:
                    ...
                    print(f"[SEED] Updated user: {username}")
...
async def _ensure_rbac_id_defaults() -> None:
    if engine.dialect.name != "postgresql":
        return
    ...
    async with engine.begin() as conn:
        try:
            ...
            await conn.execute(text("ALTER TABLE permissions ALTER COLUMN id SET DEFAULT nextval('permissions_id_seq')"))
            await conn.execute(text("SELECT setval('permissions_id_seq', COALESCE((SELECT MAX(id) FROM permissions), 1), true)"))
        except Exception:
            pass

        try:
            await conn.execute(text("CREATE SEQUENCE IF NOT EXISTS roles_id_seq"))
            await conn.execute(text("ALTER TABLE roles ALTER COLUMN id SET DEFAULT nextval('roles_id_seq')"))
            await conn.execute(text("SELECT setval('roles_id_seq', COALESCE((SELECT MAX(id) FROM roles), 1), true)"))
        except Exception:
            pass
        ...
        try:
            ...
            await conn.execute(text("SELECT setval('users_id_seq', COALESCE((SELECT MAX(id) FROM users), 1), true)"))
        except Exception:
            pass

async def _ensure_employee_columns_exist() -> None:
    employee_columns = {
    ...
    async with engine.begin() as conn:
        try:
            ...
            for col, ddl in employee_columns.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE employees ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_employee_warning_columns_exist() -> None:
    cols = {
        "found_with": "TEXT",
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(employee_warnings)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in cols.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE employee_warnings ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_client_site_guard_allocation_employee_fk() -> None:
    if engine.dialect.name != "postgresql":
        return
...
async def _ensure_employee_attachment_columns_exist() -> None:
    cols = {
...
async def _ensure_general_item_columns_exist() -> None:
    cols = {
        "image_url": "VARCHAR",
    }
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(general_items)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in cols.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE general_items ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_client_guard_requirement_columns_exist() -> None:
    cols = {
        "start_date": "DATE",
        "end_date": "DATE",
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(client_guard_requirements)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in cols.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE client_guard_requirements ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_attendance_columns_exist() -> None:
    attendance_columns = {
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(attendance_records)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in attendance_columns.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE attendance_records ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_vehicle_columns_exist() -> None:
    vehicle_columns = {
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(vehicles)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in vehicle_columns.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE vehicles ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_vehicle_assignment_columns_exist() -> None:
    assignment_columns = {
        "route_stops": "TEXT",
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(vehicle_assignments)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in assignment_columns.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE vehicle_assignments ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_payroll_payment_status_columns_exist() -> None:
    payroll_columns = {
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(payroll_payment_status)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in payroll_columns.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE payroll_payment_status ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_payroll_sheet_entry_columns_exist() -> None:
    cols = {
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(payroll_sheet_entries)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in cols.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE payroll_sheet_entries ADD COLUMN {col} {ddl}"))
        except Exception:
            pass

async def _ensure_payroll_sheet_entry_employee_fk() -> None:
    if engine.dialect.name != "postgresql":
        return
...
async def _ensure_client_site_guard_allocation_columns_exist() -> None:
    cols = {
        "site_id": "INTEGER",
        "contract_id": "INTEGER",
    ...
    async with engine.begin() as conn:
        try:
            existing = set()
            if engine.dialect.name == "sqlite":
                result = await conn.execute(text("PRAGMA table_info(client_site_guard_allocations)"))
                rows = result.fetchall()
                existing = {r[1] for r in rows}
            else:
                result = await conn.execute(
            ...
            for col, ddl in cols.items():
                if col in existing:
                    continue
                await conn.execute(text(f"ALTER TABLE client_site_guard_allocations ADD COLUMN {col} {ddl}"))
        except Exception:
            pass
...
async def run_startup_tasks():
    await _ensure_rbac_id_defaults()
    try:
        await _seed_rbac()
    except Exception:
        pass

    await _ensure_client_name_is_text()
    await _ensure_employee_columns_exist()
    await _ensure_employee_warning_columns_exist()
    await _ensure_client_site_guard_allocation_employee_fk()
    await _ensure_employee_attachment_columns_exist()
    await _ensure_general_item_columns_exist()
    await _ensure_client_guard_requirement_columns_exist()
    await _ensure_attendance_columns_exist()
    await _ensure_vehicle_columns_exist()
    await _ensure_vehicle_assignment_columns_exist()
    await _ensure_payroll_payment_status_columns_exist()
    await _ensure_payroll_sheet_entry_columns_exist()
    await _ensure_payroll_sheet_entry_employee_fk()
    await _ensure_client_site_guard_allocation_columns_exist()
