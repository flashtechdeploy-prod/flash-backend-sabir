...
async def export_accounts_monthly_pdf(
    ...
    # I should probably import it if needed or use Employee.
...
async def export_employee_inventory_pdf(
    ...
    if search:
        ...
        query = query.filter(
            ...
            (Employee.fss_no.ilike(s)) |
            (Employee.serial_no.ilike(s))
        )
    
    employees = query.order_by(Employee.serial_no.asc()).all()
...
async def export_single_employee_inventory_pdf(
    ...
    emp = db.query(Employee).filter((Employee.fss_no == employee_id) | (Employee.serial_no == employee_id) | (Employee.id == (int(employee_id) if employee_id.isdigit() else -1))).first()
    if not emp:
