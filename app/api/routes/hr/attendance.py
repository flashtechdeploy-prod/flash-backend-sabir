...
async def export_employee_attendance_pdf(
    ...
    emp2 = db.query(Employee).filter(
        (Employee.fss_no == employee_id) | 
        (Employee.serial_no == employee_id) | 
        (Employee.id == (int(employee_id) if employee_id.isdigit() else -1))
    ).first()
...
async def export_attendance_pdf(
    ...
    if from_date and to_date:
        ...
        emp_q = db.query(Employee).filter(Employee.created_at <= cutoff)
        if department:
            emp_q = emp_q.filter(Employee.category == department)
        if designation:
            emp_q = emp_q.filter(Employee.designation == designation)
        if search and search.strip():
            ...
            emp_q = emp_q.filter(
                or_(
                    ...
                    Employee.serial_no.ilike(q),
                    Employee.fss_no.ilike(q),
                    Employee.unit.ilike(q),
                    Employee.category.ilike(q),
                )
            )
        employees = emp_q.order_by(Employee.id.asc()).all()
        attendance = (
            db.query(AttendanceRecord)
