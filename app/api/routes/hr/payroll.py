...
async def upsert_payment_status(
    ...
    emp = db.query(Employee).filter(Employee.fss_no == payload.employee_id).first()
...
async def payroll_report(
    ...
    employees = (
        db.query(Employee)
        .filter(or_(Employee.created_at == None, Employee.created_at <= cutoff))
        .order_by(Employee.serial_no.asc())
        .all()
    )
...
async def bulk_upsert_payroll_sheet_entries(
    ...
    for e in payload.entries:
        ...
        # Sync with master employee profile (Employee)
        emp = db.query(Employee).filter(Employee.id == e.employee_db_id).first()
        if emp:
            ...
            # Based on employees2.py and Employee.py, there isn't a top-level bank_name 
