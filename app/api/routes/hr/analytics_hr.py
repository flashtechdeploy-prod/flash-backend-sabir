...
def _attendance_employee_key(employee: Employee) -> str:
...
def _find_legacy_employee_id(db: Session, employee: Employee) -> Optional[str]:
...
def _get_employee_inventory(db: Session, employee: Employee) -> List[InventoryItem]:
...
def _calculate_attendance_summary(db: Session, employee: Employee, deactivation_date: str) -> AttendanceSummary:
...
def _calculate_salary(db: Session, employee: Employee, attendance: AttendanceSummary) -> SalaryCalculation:
...
async def move_to_inactive(pending_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Get employee
        employee = db.query(Employee).filter(Employee.id == pending["employee_db_id"]).first()
        if not employee:
            raise HTTPException(status_code=404, detail="Employee not found")
