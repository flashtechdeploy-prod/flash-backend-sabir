...
@router.get("/summary", response_model=GeneralInventorySummary)
async def get_summary(db: Session = Depends(get_db)) -> GeneralInventorySummary:
    """Get summary statistics for general inventory."""
    items = db.query(GeneralItem).all()
    
    total_items = len(items)
    total_value = sum((i.quantity_on_hand or 0) * (i.unit_cost or 0) for i in items)
    low_stock_count = sum(1 for i in items if i.min_quantity and i.quantity_on_hand <= i.min_quantity)
    out_of_stock_count = sum(1 for i in items if (i.quantity_on_hand or 0) == 0)
    
    return GeneralInventorySummary(
